...
?>
<h2>Select Date</h2>
<?php echo form_open('admin/slot');

?>
Date : <input type="text" name="date" value="" id="date"  /><br />
          <table align="left">
...
foreach($task as $now_task)
{
	...
	if($now_task[0]<>'id')
	{
		...
		if($z==0)
		{
			echo '<tr><td align=center><span class="small">'.$options['slot'.$x].'</span></td>';
			if('slot'.$x==$now_task[7])
			{
				echo '<td align =center><a href="detail_task/'.$now_task[0].'"><strong>'.$now_task[1].'</strong></a>';
			}
			else
			{
				echo '<td align =center><span class="small">empty!</td>';
			}
			//echo '<tr><td align=center><span class="small"><a href="detail_task/'.$now_task[0].'">'.$options[$now_task[7]].'</span></td><td align =center><a href="detail_task/'.$now_task[0].'">'.$now_task[1];
			echo '</tr>';
		}
	}
	}
