...
foreach($task as $now_task)
{
	if($x>1)
	{
		echo '<tr><td><span class="small"><a href="detail_task/'.$now_task[0].'">'.$now_task[0].'</span></td><td><a href="detail_task/'.$now_task[0].'">'.$now_task[1].'</td><td><a href="detail_task/'.$now_task[0].'">';
		$u = explode("?",$now_task[3]);
		foreach($u as $t) echo '<span class="small">'.$t.br(1).'</span>';
		$now_task[4] = character_limiter($now_task[4], 25);
		echo '</td><td><span class="small">'.$now_task[4].'</span></td><a href="detail_task/'.$now_task[0].'">';
		echo '<td><span class="small">'.$now_task[6];
		echo '</span></td></tr>';
	}
	$x++;
}
